...
func (m *Meta) backendFromConfig(opts *BackendOpts) (backend.Backend, tfdiags.Diagnostics) {
	...
	case stateStoreConfig == nil && !s.StateStore.Empty() &&
		...
		// TODO: Require m.migrateState == true before migration? -> check what we do for backends

		return m.stateStore_c_S(s.StateStore, sMgr, opts)
...
func (m *Meta) stateStore_c_S(ssCfgState *workdir.StateStoreConfigState, ssSMgr *clistate.LocalState, opts *BackendOpts) (backend.Backend, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	ss, moreDiags := m.savedStateStore(ssSMgr, opts.ProviderFactory)
	diags = diags.Append(moreDiags)
	if moreDiags.HasErrors() {
		return nil, diags
	}
	...
	// TODO: Consider output bool condition here
